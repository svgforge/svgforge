...
import SVGSpriteLayouter from '../../lib/svg-sprite/layouter.js';
...
const mockLayout = createMock();
const mockModes = {
  css: createMock().mockImplementation(() => ({layout: mockLayout})),
  defs: createMock().mockImplementation(() => ({layout: mockLayout})),
  stack: createMock().mockImplementation(() => ({layout: mockLayout})),
  symbol: createMock().mockImplementation(() => ({layout: mockLayout})),
  view: createMock().mockImplementation(() => ({layout: mockLayout})),
};
setDependency('layouter:layoutModes', mockModes);

describe('testing layout()', () => {
  let spriter;
  const noop = createMock();

  const layouters = mockModes;
...
describe('testing constructor', () => {
  ...
  it('should fill up shapes with file sizes if config.example is true', () => {
    ...
    const TEST_SHAPE = {
      ...
      base: 'base',
      master: {id: 'master'},
      getDimensions: createMock().mockReturnValueOnce({}),
  ...
  it('should fill up shapes accordingly to spriter shapes '
    ...
    const TEST_MASTER_SHAPE = {
      ...
      base: 'base',
      master: {id: 'master'},
      getDimensions: createMock().mockReturnValueOnce(TEST_DIMENSIONS['100x100']),
    ...
    const TEST_FOURTH = {
      ...
      base: 'base',
      master: null,
      getDimensions: createMock().mockReturnValueOnce(TEST_DIMENSIONS['200x200']),
      config: {spacing: {padding: TEST_PADDINGS['0']}},
    };
    const TEST_FIFTH = {
      id: '5',
    ...
    const TEST_SEVENTH = {
      ...
      base: 'base',
      master: null,
      getDimensions: createMock().mockReturnValueOnce(TEST_DIMENSIONS['300x300']),
      config: {spacing: {padding: TEST_PADDINGS['0']}},
    };
    const TEST_EIGHT = {
      id: '5',
...
describe('testing defaultVariables', () => {
  ...
  describe('testing invert', () => {
    ...
    it('should revert number as a mustache function', () => {
      ...
      const TEST_NUMBER = 5;
      const MOCK_RENDER_FN = createMock().mockReturnValueOnce(TEST_NUMBER);
  ...
  describe('testing classname', () => {
    ...
    it('should call render as a mustache function', () => {
      ...
      const TEST_STR = 'test';
      const MOCK_RENDER_FN = createMock().mockReturnValueOnce(TEST_STR);
    ...
    describe('testing function', () => {
      it('should replace all last class if string includes spaces', () => {
        ...
        expect(layouter._commonData.classname()(
          TEST_STR,
          createMock().mockReturnValueOnce(TEST_STR),
        )).toBe('test4');
      });
  ...
  describe('testing escape', () => {
    ...
    it(String.raw`should revert number replace \ with \\ as a mustache fn`, () => {
      expect.hasAssertions();

      const TEST_STR = String.raw`\1\2\3\4`;
      const MOCK_RENDER_FN = createMock().mockReturnValueOnce(TEST_STR);

      expect(layouter._commonData.escape()(TEST_STR, MOCK_RENDER_FN)).toBe(String.raw`\\1\\2\\3\\4`);
      expect(MOCK_RENDER_FN).toHaveBeenCalledWith(TEST_STR);
    });
  });
