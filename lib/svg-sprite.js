  ...
  */
  ...
  // If the shape queue is currently active return early
    ...
    if (Object.keys(config).length === 0) {
      ...
      const mainShapes = this._shapes.filter(shape => !shape.master);
      this.info('Compiling %d shapes...', mainShapes.length);

      // Initialize the namespace powers
      while (this._namespacePow.length === 0 || (26 ** this._namespacePow.length < mainShapes.length)) {
      ...
      for (const [index, shape] of mainShapes.entries()) {
