...
import {isObject, deepMerge, runParallelLimit} from '../utils/index.js';
...
/**
  ...
  _loadHTMLExamplePartials() {
    ...
    const readPartial = file => {
      try {
        return getDependency('node:fs:readFileSync', defaultFs.readFileSync).call(defaultFs, path.resolve(folder, path.join('tmpl', 'common', file)), 'utf8');
      } catch {
        return '';
      }
    };
