...
class SVGSpriteCss extends SVGSpriteBase {
  ...
  /**
   ...
   @returns {string} Vertical layout identifier
   */
  get LAYOUT_VERTICAL() {
    return 'vertical';
  }
  ...
  /**
   ...
   @returns {string} Horizontal layout identifier
   */
  ...
  /**
   ...
   @returns {string} Diagonal layout identifier
   */
  get LAYOUT_DIAGONAL() {
    return 'diagonal';
  }
  ...
  /**
   ...
   @returns {string} Packed layout identifier
   */
  get LAYOUT_PACKED() {
    return 'packed';
  }
  ...
  // Prepare the dimension suffix
    ...
    // String: Use the given string as mixin name
      case 'string': {
        this.config.mixin = this.config.mixin.trim().length > 0 ? this.config.mixin.trim() : null;
        break;
      }
 ...
 */
  // Polymorphic hook (overridden or dispatched cross-class); `#private` would break dispatch.
  // eslint-disable-next-line unicorn/prefer-private-class-fields, complexity -- _layout orchestrates CSS layout phases (pseudo-classes, bin-packing, offsets) sequentially with shared state.
  _layout() {
    ...
    // Skip non-master shapes for all but orthogonal layouts
      ...
      if (!shape.master) {
        positionMap[this._spriter._shapes[index].id] = {...shape.position.absolute};
      }
 ...
 @param {object} pseudoShapeMap Map of shape IDs needing a :regular pseudo class
    ...
    for (const [index, shape] of this._spriter._shapes.entries()) {
      if (this._displaceable || !shape.master) {
        this._addShapeToSimpleCssSprite(
          ...
          index,
          // eslint-disable-next-line no-bitwise -- First/last shape flags are encoded as a bitmask.
 ...
 */
  ...
  // Prepare the selectors
    const selector = {
      shape: needsRegular || shape.state
        ? [{
          ...
          expression: format(this.config.prefix, String.raw`${shape.base}\:${shape.state || 'regular'}`),
    ...
    this.data.shapes[index] = {
      ...
      dimensions: {
        ...
        extra: Boolean(isString(this.config.dimensions) && this.config.dimensions.length > 0),
      },
    };
 ...
 @param {string} xmlDeclaration String prepended to the sprite
 @param {string} doctypeDeclaration String prepended after the XML declaration
 ...
 */
  ...
  _buildSVG(xmlDeclaration, doctypeDeclaration) {
    ...
    svg.add(Object.values(this.data.shapes).map(value => value.svg));
