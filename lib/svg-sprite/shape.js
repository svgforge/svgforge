...
SVGShape.prototype._initSVG = function () {
  ...
  const validSVGRegExp = /<svg(?:\s+[\d\-:a-z]+=(["']).*?\1)*\s*(?:(\/)|(>[\s\S]*<\/svg))>/i;
  ...
  }).parseFromString(this.svg.current, 'image/svg+xml');
...
SVGShape.prototype._replaceIdAndClassnameReferences = function (str, substIds, substClassnames, selectors) {
  ...
  return selectors ? this._replaceIdAndClassnameReferencesInCssSelectors(str, getDependency('cssom', cssom).parse(str).cssRules, substIds, substClassnames) : str;
};
    ...
    if (selText === undefined) {
      ...
      const collectRule = rule => {
        for (const item of rule.items) {
          if (item.type === 'Id' && substIds !== null && `#${item.name}` in substIds) {
            ids.push(item.name);
          } else if (item.type === 'ClassName' && substClassnames !== null && `.${item.name}` in substClassnames) {
            classnameFilter(item.name);
          }
        }

        if (rule.nestedRule) {
          collectRule(rule.nestedRule);
        }
      };
